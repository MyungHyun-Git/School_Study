...
char buffer2[100];
char buffer3[100] = "내이름은 이명현이야 \n";
char buffer4[100] = "내 나이는 23살이야 \n";
...
int main()
{
    ...
    while(1)
		...
		while(1)
		{
			...
			if(strncasecmp(buffer2, "이름이 뭐야?", 17)==0)
		      		  {
					buffer2[c]='\0';
					write(c_socket, buffer3, strlen(buffer3));

		    		  }
			if(strncasecmp(buffer2, "몇 살이야?" , 14) == 0)
			{
				buffer2[c]='\0';
				write(c_socket, buffer4, strlen(buffer4));
			}
