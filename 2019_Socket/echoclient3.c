...
int main()
{
	...
	char rcv3buffer[BUFSIZE]={0}; //서버에서 온 문자열 일부분을 저장.
    ...
    while(1)
	{
		...
		while(1)
		{
			...
			if(strncasecmp(rcv2buffer, "strlen ", 7) == 0)
			{
				rcv2buffer[k]='\0';
				p=read(c_socket, rcv3buffer, sizeof(rcv3buffer));
				rcv3buffer[p]='\0';
				part=strlen(rcv3buffer)-1;
				printf("내가 보낸 문자열의 길이는 : %d \n",part);
